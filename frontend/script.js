 ...
 * GET    /api/movies/recommendations             — Load recommended movies based on saved preferences
...
function clearRecommendationHeader() {
  document.querySelector('.recommendation-heading')?.remove();
  document.querySelector('.recommendation-source')?.remove();
}

function setRecommendationSource(message) {
  document.querySelector('.recommendation-source')?.remove();

  if (!message || !movieContainer) {
    return;
  }

  movieContainer.insertAdjacentHTML(
    'beforebegin',
    `<p class="recommendation-source">${message}</p>`
  );
}
...
function setRecommendationHeading() {
  ...
  movieContainer.insertAdjacentHTML(
    'beforebegin',
    '<h2 class="recommendation-heading">Recommended Movies</h2>'
  );
}
...
async function savePreferences(event) {
  ...
  try {
    ...
    await loadRecommendedMoviesOnOpen();
  } catch (error) {
    if (message) {
...
async function renderSuggestions(source, suggestions) {
  ...
  setRecommendationSource(`Recommendations generated by: <strong>${sourceText}</strong>`);
}

async function loadRecommendedMoviesOnOpen() {
  if (!movieContainer) return;

  setRecommendationHeading();
  renderStatus('Loading recommended movies...');
  ...
  try {
    ...
    const limitedMovies = Array.isArray(movies) ? movies.slice(0, 10) : [];

    if (limitedMovies.length === 0) {
      setRecommendationSource('Recommendations based on your saved preferences');
      renderStatus('No recommendations yet. Save some genres in Preferences first.');
      return;
    }
    ...
    clearRecommendationHeader();
    renderStatus('Failed to load recommendations.');
  }
}
...
// Load personalized recommendations on first paint.
loadRecommendedMoviesOnOpen();
